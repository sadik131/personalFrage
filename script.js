...
document.addEventListener("DOMContentLoaded", function () {
    console.log("Initializing flatpickr...");  // Debug message
...
// handle toggle radio button
function toggleRadio(event, radioId) {
    let radioButton = document.getElementById(radioId);

    // Check if the target is the input (radio button)
    if (event.target.tagName === 'INPUT') {
        console.log("Clicked on input");
        console.log("Current state:", radioButton.checked);

        // Toggle the checked state manually
        if (radioButton.checked) {
        //     console.log("Unchecking the radio button");
            radioButton.checked = false;} // Uncheck if it's already checked
        // } else {
        //     console.log("Checking the radio button");
        //     radioButton.checked = true;  // Check if it's unchecked
        // }

        // console.log("New state:", radioButton.checked);
        return; // Exit the function, no need to prevent default for input
    }

    // If it's clicked on the label, prevent default behavior
    event.preventDefault();

    // Manually toggle the checked state of the radio button
